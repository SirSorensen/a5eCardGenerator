...
class TableToDataStructure:
    ...
    def make_data_structures(self, table_extractor : TableInterpreter) -> list[Context]:
        ...
        card_data_list : list[Context] = []
        
        for i in range(len(list_of_names)):
            print(f"Making a {self.card_type} data structure for {list_of_names[i][0]}...")
            os_filepath = f"Outputs\{self.card_type}s\source_text_" + Context.name_to_data_name(list_of_names[i][0]) + r".html"
            ...
            if self.card_type == Spell.__name__:
                context = self.__make_card_data(list_of_names[i][0], list_of_names[i][1], should_scrape_source_text, list_of_all_summaries[i])
            else:
                context = self.__make_card_data(list_of_names[i][0], list_of_names[i][1], should_scrape_source_text)

            card_data_list.append(context)

        return card_data_list

    # This function makes a card data structure
    def __make_card_data(self, name : str, url_ending : str, should_scrape_source_text : bool, summary : str = "") -> Spell|Feat|CombatManeuver|Context:       
