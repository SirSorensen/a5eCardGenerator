...
class DataStructureExtractor(CodeInterpreter):
    ...
    def extract_field_information(self, field_class : str = "", field_id : str = "") -> list[str]|str:
        ...
        if field_div:
            ...
            if item_fields:
                return CodeInterpreter.try_unwrap_list([CodeInterpreter.get_text(item) for item in item_fields])
            
            # Concatenate the text within <p> tags while preserving line breaks
            paragraphs = field_div.find_all('p')
            if paragraphs:
                return CodeInterpreter.try_unwrap_list(['\n'.join(CodeInterpreter.get_text(p) for p in paragraphs)])
            
            spans = field_div.find_all('span')
            if spans:
                return CodeInterpreter.try_unwrap_list([CodeInterpreter.get_text(s) for s in spans])


            return CodeInterpreter.try_unwrap_list([CodeInterpreter.get_text(field_div)])
    ...
    def extract_list_of_names(self) -> list[str]:
        ...
        return CodeInterpreter.try_unwrap_list([CodeInterpreter.get_text(item) for item in field_td])
